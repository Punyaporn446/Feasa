...
begin
     ...
     begin
          ...
          if not (FeasaCom_Sequence_FindTestSettings(DevicePort, TotalLEDCount, Fiber, SignalSpeed, BlinkingSpeed, MinCycleCount, TimeResolutionIsImportant, @CaptureTime, @WaitTime, @SampleCount)=1) then
          begin
...
begin
     ...
     begin
          ...
          resp := FeasaCom_Sequence_Capture(DevicePort, Fiber);
          if not (resp = 1) then
          begin
             FeasaCom_GetError_Description(PAnsiChar(buffer));
             MessageDlg(buffer, mtWarning, [mbOK], 0);
          ...
          resp := FeasaCom_Sequence_ReadxyI(DevicePort, Fiber, @xValues, @yValues, @IntensityValues);
          if not (resp = 1) then
          begin
             FeasaCom_GetError_Description(PAnsiChar(buffer));
             MessageDlg(buffer, mtWarning, [mbOK], 0);
