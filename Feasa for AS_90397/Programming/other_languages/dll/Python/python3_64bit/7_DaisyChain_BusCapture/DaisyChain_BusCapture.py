...
class Application(ttk.Frame):
	...
	def __init__(self, master=None):
		...
		self.FeasaCom_DaisyChain_Add = self.FeasaDLL['FeasaCom_DaisyChain_Add']
		self.FeasaCom_DaisyChain_Add.argtypes = [ctypes.c_int, ctypes.c_char_p]
		self.FeasaCom_DaisyChain_Add.restype = ctypes.c_int
		self.FeasaCom_DaisyChain_Capture = self.FeasaDLL['FeasaCom_DaisyChain_Capture']
		self.FeasaCom_DaisyChain_Capture.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int]
	...
	def createWidgets(self):
		# controls
		self.frameSetup = ttk.LabelFrame(self, text='Communications Setup', width='183', height='325')
		...
		self.lstAnalysers = tk.Listbox(self.frameSetup, width='26', height='5')
		self.frameCommands = ttk.LabelFrame(self, text='Commands', width='270', height='190')
		...
		self.frameResponse = ttk.LabelFrame(self, text='Response', width='270', height='130')
	...
	def btnRead_Click(self):
		...
		if ret == 1:
			...
			for i in range(0, self.lstAnalysers.size()):
				ret = self.FeasaCom_DaisyChain_Add(port, self.lstAnalysers.get(i).encode())

			# Set default timeout to 10 seconds
			self.FeasaCom_SetResponseTimeout(10000)
			...
			ret = self.FeasaCom_DaisyChain_Capture(port, 0, CaptureRange, 0)
			if ret == -1:
			...
			for i in range(0, self.lstAnalysers.size()):
				...
				ret = self.FeasaCom_Send(port, b'BUSGET' + self.lstAnalysers.get(i).encode(), buffer)
				if ret == -1:
					messagebox.showwarning('Error', 'Unable to send the command!')
					self.FeasaCom_Close(port)
					return
