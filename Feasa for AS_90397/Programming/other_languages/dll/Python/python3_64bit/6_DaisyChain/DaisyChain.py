...
class Application(ttk.Frame):
	...
	def createWidgets(self):
		...
		self.frameDC = ttk.LabelFrame(self, text='Daisy Chain', width='178', height='130')
		self.lblSN = ttk.Label(self.frameDC, text='Serial Number of the LED Analyser you want to control:', wraplength='160', justify='left')
		self.txtSN = ttk.Entry(self.frameDC, width='6')
		self.btnDCGet = ttk.Button(self.frameDC, text='GET BUS', command=self.btnBusGet_Click, width='11')
		self.btnDCFree = ttk.Button(self.frameDC, text='FREE BUS', command=self.btnBusFree_Click, width='11')
		self.frameMeasure = ttk.LabelFrame(self, text='Measure', width='178', height='90')
		...
		self.frameResponse = ttk.LabelFrame(self, text='Response', width='178', height='100')
		self.txtLog = tk.Text(self.frameResponse, wrap='word', width='20', height='4', background='white', borderwidth='1')
		...
		self.btnDCGet.grid(row=2, column=0, pady=3)
		self.btnDCFree.grid(row=2, column=1, pady=3)
	...
	def btnBusGet_Click(self):
		...
		if ret == 1:
			...
			ret = self.FeasaCom_Send(port, b'BUSGET' + serial_number.encode(), buffer)
			if ret == -1:
			...
			# shows info text
			self.txtLog.insert(tk.END, buffer.value.decode('ascii') + '\r\nThe Bus belongs to the LED Analyser with SN: ' + serial_number)
	...
	def btnBusFree_Click(self):
		...
		if ret == 1:
			...
			# Send command to the LED Analyser
			# You can notice that there is no need to send the CR + LF characters (The command Send does it for you automatically)
			ret = self.FeasaCom_Send(port, b'BUSFREE', buffer)
			if ret == -1:
				messagebox.showwarning('Error', 'Unable to send the command!')
				self.FeasaCom_Close(port)
				return
