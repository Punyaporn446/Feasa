...
class Application(ttk.Frame):
	...
	def __init__(self, master=None):
		...
		self.FeasaCom_GetBaudrate = self.FeasaDLL['FeasaCom_GetBaudrate']
		self.FeasaCom_GetBaudrate.argtypes = [ctypes.c_int]
	...
	def createWidgets(self):
		...
		self.lblInfo = ttk.Label(self.frameSetup, text='This is an example of how we can select the proper baudrate for the Analyser. We also have the option to detect it automatically, however will take more time to open the port (this is clearly noticed when the baudrate of the LED Analyser is different than 57600).', wraplength='165', justify='left')
		...
		self.txtLog = tk.Text(self.frameCommands, wrap='word', width='20', height='6', background='white', borderwidth='1')
	...
	def btnRead_Click(self):
		...
		if self.BaudMode.get() == 0:
			ret = self.FeasaCom_Open(port, b"AUTO")
		else:
			ret = self.FeasaCom_Open(port, self.lstBaudrate.get().encode())
		...
		if ret == 1:
			...
			if self.BaudMode.get() == 0:
				self.txtLog.insert(tk.END, '\r\n' + 'Baudrate: ' + str(self.FeasaCom_GetBaudrate(port)))
