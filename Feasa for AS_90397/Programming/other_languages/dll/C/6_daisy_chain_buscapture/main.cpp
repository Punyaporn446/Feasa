...
#define COMPORT 56 // This is the comm port where the LED Analyser is connected to
...
int main(int argc, char* argv[])
{
	...
	for (i=0; i<BusAnalysers_Count; i++) {
		resp = FeasaCom_DaisyChain_Add(COMPORT, BusAnalysers[i]);
		if ( resp==0 ) {
	...
	printf("Capturing...");
	resp = FeasaCom_DaisyChain_Capture(COMPORT, 0, capture_range, 0);
	if ( resp==-1 ) {
		printf(" Failed!\n");
		FeasaCom_Close(COMPORT);
		FreeLibrary((HMODULE)hFeasaComDLL);
	...
	// Free BUS
	printf("Free BUS...");
	resp = FeasaCom_Send(COMPORT, "BUSFREE", buffer);
	if ( resp==-1 ) {
		printf(" Failed!\n");
		FeasaCom_Close(COMPORT);
		FreeLibrary((HMODULE)hFeasaComDLL);
