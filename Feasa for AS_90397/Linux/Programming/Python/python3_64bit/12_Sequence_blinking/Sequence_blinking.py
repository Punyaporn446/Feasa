...
#  Developed by: Carles Martinez Rius
#
...
class Application(ttk.Frame):
	...
	def createWidgets(self):
		...
		self.lblInfo = ttk.Label(self.frameSetup, text='This example explores the Sequence functionality of the LED Analyser, by allowing the user to perform a Sequence test, and also to find some possible test parameters through the functions provided in the SO Library.\n\nFor simplicity, this example is limited to test one channel at a time, but the LED Analuser and SO Library can measure all channels simultaneously, if required.', justify='left', wraplength=210)
		...
		self.frameFindParams = ttk.LabelFrame(self, text='Commands', width='340', height='250')
		...
		self.btnFindParams = ttk.Button(self.frameFindParams, text='FIND PARAMETERS', command=self.btnFindParams_Click, width='38')
		...
		# --- Sequence Test frame ---
		self.frameSequenceTest = ttk.LabelFrame(self, text='Sequence Test', width='240', height='250')
		...
		self.btnSequenceTest = ttk.Button(self.frameSequenceTest, text='SEQUENCE TEST', command=self.btnSequenceTest_Click, width='27')
	...
	def btnFindParams_Click(self):
		...
		# get selected port
		DevicePath = ('/dev/' + self.lstPorts.get()).encode('ascii')
		...
		# open port
		ret = FeasaCom.Open(DevicePath, 0)
		if ret == 1:
			#No error
			...
			ret = FeasaCom.Sequence_FindTestSettings(DevicePath, TotalLEDCount, Fiber, SignalSpeed, BlinkingSpeed, MinCycleCount, TimeResolutionIsImportant, CaptureTime, WaitTime, SampleCount)
			if ret != 1:
	...
	def btnSequenceTest_Click(self):
		...
		if ret == 1:
			...
			# Set up Sequence
			ret = FeasaCom.Sequence_Setup(DevicePath, StartDelay, CaptureTime, WaitTime, SampleCount, TOFLASH)
			if ret != 1:
				FeasaCom.GetError_Description(buffer)
				messagebox.showwarning('Error', buffer.value.decode('ascii'))
				FeasaCom.Close(DevicePath)
			...
			ret = FeasaCom.Sequence_Capture(DevicePath, Fiber)
			if ret != 1:
				FeasaCom.GetError_Description(buffer)
				messagebox.showwarning('Error', buffer.value.decode('ascii'))
				FeasaCom.Close(DevicePath)
			...
			# Read back results
			ret = FeasaCom.Sequence_ReadxyI(DevicePath, Fiber, xValues, yValues, IntensityValues)
			if ret != 1:
				FeasaCom.GetError_Description(buffer)
				messagebox.showwarning('Error', buffer.value.decode('ascii'))
				FeasaCom.Close(DevicePath)
