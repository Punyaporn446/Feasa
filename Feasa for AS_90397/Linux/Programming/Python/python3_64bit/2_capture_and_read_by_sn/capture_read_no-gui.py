...
# define port number
serial_number = b'76A6'

# define fiber number
num_fib = 1

# open port
ret = FeasaCom_Open(serial_number, 57600)
if ret == 1:
	...
	# Send command to the LED Analyser
	# You can notice that there is no need to send the CR + LF characters (The command Send does it for you automatically)
	ret = FeasaCom_Send(serial_number, b'GETRGBI' + b'%02d'%num_fib, buffer)
	if ret == -1:
		print ('Unable to send the command!')
		FeasaCom_Close(serial_number)
		quit()
