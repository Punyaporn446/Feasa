...
class Application(ttk.Frame):
	...
	def createWidgets(self):
		...
		# --- Sequence Test frame ---
		self.frameSequenceTest = ttk.LabelFrame(self, text='Sequence Test', width='270', height='250')
		...
		self.txtTimes = tk.Text(self.frameResponse, wrap='word', width='31', height='12', background='white', borderwidth='1')
	...
	def btnSequenceTest_Click(self):
		...
		if ret == 1:
			...
			ret = FeasaCom.Sequence_Capture(DevicePath, 0) #0: test all fibers
			if ret != 1:
				FeasaCom.GetError_Description(buffer)
				messagebox.showwarning('Error', buffer.value.decode('ascii'))
				FeasaCom.Close(DevicePath)
			...
			for i in range(0, FibersToTest):
				ret = FeasaCom.Sequence_ReadIntensity(DevicePath, i + 1, IntensityValues)
				if ret != 1:
					FeasaCom.GetError_Description(buffer)
					messagebox.showwarning('Error', buffer.value.decode('ascii'))
					FeasaCom.Close(DevicePath)
			...
			# Retrieve LED times
			ret = FeasaCom.Sequence_GetSweepingPattern(DevicePath, FibersToTest, isOffToOnPattern, LowTimes, HighTimes, tIntensityValues)
			if ret != 1:
				FeasaCom.GetError_Description(buffer)
				messagebox.showwarning('Error', buffer.value.decode('ascii'))
				FeasaCom.Close(DevicePath)
