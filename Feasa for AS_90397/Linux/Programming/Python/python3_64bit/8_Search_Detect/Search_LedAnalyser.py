...
class Application(ttk.Frame):
	...
	def createWidgets(self):
		# controls
		self.frameInfo = ttk.LabelFrame(self, text='Info', width='190', height='345')
		...
		self.frameSearch = ttk.LabelFrame(self, text='Search', width='250', height='100')
		...
		self.btnSearch = ttk.Button(self.frameSearch, text='FIND LED ANALYSER &\n SHOW THE STATUS', command=self.btnSearch_Click, width='28')
		self.frameStatus = ttk.LabelFrame(self, text='Status', width='250', height='240')
		self.txtLog = tk.Text(self.frameStatus, width='32', height='14', background='white', borderwidth='1', font=('Courier','9'))
	...
	def btnDetect_Click(self):
		...
		[DevPaths, pDevPaths] = self.CreateCArrayOfStrings(50, 255)

		#Perform detection
		nDetected = self.FeasaCom_Detect(pDevPaths, 0);
		if nDetected>0:
			self.txtLog.insert(tk.END, 'Devices detected:\r\n')
			for i in range(0, nDetected):
				self.txtLog.insert(tk.END, '...on port ' + DevPaths[i].value.decode('ascii') + '\r\n')
		else:
	...
	def btnDetectSerials_Click(self):
		...
		#Perform detection
		nDetected = self.FeasaCom_DetectSN(plstSerials, 0);
		if nDetected>0:
			self.txtLog.insert(tk.END, 'Devices detected:\r\n')
			for i in range(0, nDetected):
	...
	def btnSearch_Click(self):
		...
		DevPath = ''
		res = self.FeasaCom_IsConnected(buffer, serial_number.encode(),  57600)
		if res == -1:
			# shows info text
			...
			self.txtLog.insert(tk.END, 'LED Analyser found on port ' + DevPath.decode('ascii') + '\r\n')
		...
		if ret == 1:
			...
			ret = self.FeasaCom_Send(DevPath, b'GETSTATUS', buffer)
			if ret == -1:
				messagebox.showwarning('Error', 'Unable to send the command!')
