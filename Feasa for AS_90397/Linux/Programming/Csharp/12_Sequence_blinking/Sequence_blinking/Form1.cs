    ...
    public partial class Form1 : Form
    {
        ...
        private void btnFindParams_Click(object sender, EventArgs e)
        {
            ...
            if (FeasaCom.Open(DevicePath, 0) == 1)
            {
                ...
                resp = FeasaCom.Sequence_FindTestSettings(DevicePath, TotalLEDCount, Fiber, SignalSpeed, BlinkingSpeed, MinCycleCount, TimeResolutionIsImportant, ref CaptureTime, ref WaitTime, ref SampleCount);
                if (resp != 1)
                {
        ...
        private void btnSequenceTest_Click(object sender, EventArgs e)
        {
            ...
            if (FeasaCom.Open(DevicePath, 0) == 1)
            {
                ...
                resp = FeasaCom.Sequence_Capture(DevicePath, Fiber);
                if (resp != 1)
                {
                    FeasaCom.GetError_Description(buffer);
                    MessageBox.Show(this, buffer.ToString(), "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                ...
                resp = FeasaCom.Sequence_ReadxyI(DevicePath, Fiber, xValues, yValues, IntensityValues);
                if (resp != 1)
                {
                    FeasaCom.GetError_Description(buffer);
                    MessageBox.Show(this, buffer.ToString(), "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
