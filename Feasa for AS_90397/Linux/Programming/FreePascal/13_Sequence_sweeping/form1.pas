...
begin
     ...
     begin
          ...
          resp := FeasaCom_Sequence_Capture(DevicePath, 0); //0: test all fibers
          if not (resp = 1) then
          begin
             FeasaCom_GetError_Description(PAnsiChar(buffer));
             MessageDlg(buffer, mtWarning, [mbOK], 0);
          ...
          begin
              ...
              resp := FeasaCom_Sequence_ReadIntensity(DevicePath, f, @IntensityValues);
              if not (resp = 1) then
              begin
                 FeasaCom_GetError_Description(PAnsiChar(buffer));
                 MessageDlg(buffer, mtWarning, [mbOK], 0);
          ...
          // Retrieve LED times
          resp := FeasaCom_Sequence_GetSweepingPattern(DevicePath, FibersToTest, isOffToOnPattern, @LowTimes, @HighTimes, @tIntensityValues); //0: test all fibers
          if not (resp = 1) then
          begin
             FeasaCom_GetError_Description(PAnsiChar(buffer));
             MessageDlg(buffer, mtWarning, [mbOK], 0);
